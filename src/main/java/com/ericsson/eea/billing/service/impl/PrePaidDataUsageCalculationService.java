...
public class PrePaidDataUsageCalculationService implements DataUsageCalculationService {
    ...
    public SubscriberBillingInfo calculateDataUsage(GetCurrentAndAvailableDataProductsResponse response) throws DatatypeConfigurationException {
        ...
        GetCurrentAndAvailableDataProductsResponse.Message.SubscriberInfo info = response.getMessage().getSubscriberInfo();
        System.out.println("MSISDN ID ==> " + info.getMsisdn());
        List<DataPass> dataPasses = response.getMessage().getDataProducts().getDataProduct();

        List<DataPass> filteredDataPasses = BillingUtils.getFilteredDataPass(dataPasses, type -> VALID_INFO_TYPE_PREPAID.contains(type.getInfoType()));
        ...
        if (calPasses == null) {
            ...
            for (int i = 0; i < calPasses.size(); i++) {
                ...
                if (i == 0) {
                    billingInfo.setBillingPeriodStartDate(toLocalDateTime(pass.getPassStartTime()).toEpochSecond(ZoneOffset.UTC));
                    billingInfo.setBillingPeriodEndDate(toLocalDateTime(pass.getPassEndTime()).toEpochSecond(ZoneOffset.UTC));
                    ...
                    billingInfo.setLbcStartDate(toLocalDateTime(pass.getPassStartTime()).toEpochSecond(ZoneOffset.UTC));
                    billingInfo.setLbcEndDate(toLocalDateTime(pass.getPassEndTime()).toEpochSecond(ZoneOffset.UTC));
                    ...
                    billingInfo.setPbcStartDate(toLocalDateTime(pass.getPassStartTime()).toEpochSecond(ZoneOffset.UTC));
                    billingInfo.setPbcEndDate(toLocalDateTime(pass.getPassEndTime()).toEpochSecond(ZoneOffset.UTC));
