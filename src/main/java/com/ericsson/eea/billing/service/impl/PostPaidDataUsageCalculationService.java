...
public class PostPaidDataUsageCalculationService implements DataUsageCalculationService {
	...
	public SubscriberBillingInfo calculateDataUsage(GetCurrentAndAvailableDataProductsResponse response)
		...
		// change pass
		List<DataPass> filteredDataPasses = filterDataPassOnFUPChange(
				getFilteredDataPass(dataPasses, type -> VALID_INFO_TYPE_POSTPAID.contains(type.getInfoType())));
		...
		LocalDateTime currentBillCycleEndDate = LocalDate.now(ZoneOffset.UTC).atTime(23, 59, 59)
				.withDayOfMonth(billingDate);
		if (billingDate < currentDate) {
			currentBillCycleEndDate = currentBillCycleEndDate.plusMonths(1);
		}

		final LocalDateTime currentBillCycleStartDate = currentBillCycleEndDate.withHour(0).withMinute(0).withSecond(01)
				.minusMonths(1).plusDays(1);
		final List<DataPass> currentCycleDataPasses = getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
				currentBillCycleStartDate, currentBillCycleEndDate);

		final DataUsageDetails currentCycleDataUsage = calculateDataUsageForCycle(currentCycleDataPasses,
		...
		final List<DataPass> previousCycleDataPasses = getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
		...
		final DataUsageDetails previousCycleDataUsage = calculateDataUsageForCycle(previousCycleDataPasses,
				subscriberInfo, BillingCycle.PREVIOUS, previousBillCycleStartDate, previousBillCycleEndDate);
		...
		final List<DataPass> penultimateCycleDataPasses = getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
				penulBillCycleStartDate, penulBillCycleEndDate);

		log.info("Final Pass for calculation -- Previous Cycle");
		penultimateCycleDataPasses.forEach(BillingUtils::printLog);

		final DataUsageDetails penultimateCycleDataUsage = calculateDataUsageForCycle(penultimateCycleDataPasses,
				subscriberInfo, BillingCycle.PENULTIMATE, penulBillCycleStartDate, penulBillCycleEndDate);

		log.info("************************* End of Calculation for Postpaid  ****************************\n");
		final SubscriberBillingInfo billingInfo = SubscriberBillingInfo.builder()
				...
				.pbcZeroRatedDataUsedPerService(penultimateCycleDataUsage.getZeroRatedDataUsedPerService()).build();
	...
	private DataUsageDetails calculateDataUsageForCycle(final List<DataPass> dataPasses, final SubscriberInfo info,
			final BillingCycle billingCycle, final LocalDateTime billingStartDate, final LocalDateTime billingEndDate) {
		...
		List<String> passToConsider = BillingCycle.CURRENT == billingCycle ? BillingConstant.CURRENT_CYCLE_INFO_TYPE
		...
		if (BillingUtils.isUnlimitedUsage(dataPasses)) {
			dataUsedUnlimited = dataPasses.stream().filter(e -> passToConsider.contains(e.getInfoType()))
					.filter(e -> e.getShareDetails() != null).map(e -> e.getShareDetails().getSharerDataUsage())
					.flatMap(Collection::stream).filter(sha -> info.getMsisdn().equals(sha.getMsisdn()))
			...
			dataUsedSharer = dataPasses.stream()
					...
					.filter(p -> BillingUtils.isSharedPass(info, p)).map(e -> e.getShareDetails().getSharerDataUsage())
					.flatMap(Collection::stream).filter(sha -> info.getMsisdn().equals(sha.getMsisdn()))
					.mapToDouble(SharerDataUsage::getUsedVolume).sum();

			if (BillingCycle.CURRENT == billingCycle) {
				Optional<DataPass> currentDataPass = dataPasses.stream().filter(e -> "C".equals(e.getInfoType()))
						.findFirst();
		...
		zeroRatedDataUsed = getZeroRatedUsage(dataPasses, BillingCycle.CURRENT == billingCycle ? "ZR" : "EZR");
		...
		return DataUsageDetails.builder().billingPeriodStartDate(billingStartDate.toEpochSecond(ZoneOffset.UTC))
				.billingPeriodEndDate(billingEndDate.toEpochSecond(ZoneOffset.UTC))
				.dataUsed(BillingUtils.getDataUsageInMB(dataUsed)).dataAvail(BillingUtils.getDataUsageInMB(dataAvail))
	...
	private double getUsageDetails(final List<DataPass> dataPasses, final Predicate<? super DataPass> predicate) {

		return dataPasses.stream().filter(e -> BillingConstant.HISTORICAL_CYCLE_INFO_TYPE.contains(e.getInfoType()))
				.filter(predicate).mapToDouble(DataPass::getVolume).sum();
	}

	private Map<String, Double> getZeroRatedDataUsedPerService(final List<DataPass> dataPasses, final String infoType) {
	...
	private Double getZeroRatedUsage(final List<DataPass> dataPasses, final String infoType) {
		...
		return dataPasses.stream().filter(e -> infoType.equals(e.getInfoType())).mapToDouble(DataPass::getVolume).sum();
	}
