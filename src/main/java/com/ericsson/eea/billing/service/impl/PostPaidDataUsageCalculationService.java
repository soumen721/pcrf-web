...
public class PostPaidDataUsageCalculationService implements DataUsageCalculationService {
    ...
    public SubscriberBillingInfo calculateDataUsage(
        ...
        List<DataPass> currentCycleDataPasses =
                filterDataPassOnFUPChange(getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
                        billCycleStartDate, billCycleEndDate));

        log.info("Final Pass after FUP remove Current Cycle");
        currentCycleDataPasses.forEach(BillingUtils::printLog);

        DataUsageDetails currentCycleDataUsage = calculateDataUsage(currentCycleDataPasses, info,
                BillingCycle.CURRENT, billCycleStartDate, billCycleEndDate);
        ...
        List<DataPass> previousCycleDataPasses =
                filterDataPassOnFUPChange(getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
                        billCycleStartDate, billCycleEndDate));
        ...
        DataUsageDetails previousCycleDataUsage = calculateDataUsage(previousCycleDataPasses, info,
                BillingCycle.PREVIOUS, billCycleStartDate, billCycleEndDate);

        // For Penultimate Period
        log.info("*************************	Penultimate Period	****************************\n");
        billCycleEndDate = billCycleEndDate.minusMonths(1);
        billCycleStartDate = billCycleEndDate.minusMonths(1).plusDays(1);
        List<DataPass> penultimateCycleDataPasses =
                filterDataPassOnFUPChange(getFilteredDataPassBasedOnBillCycle(filteredDataPasses,
                        billCycleStartDate, billCycleEndDate));
        ...
        DataUsageDetails penultimateCycleDataUsage = calculateDataUsage(penultimateCycleDataPasses, info,
                BillingCycle.PENULTIMATE, billCycleStartDate, billCycleEndDate);
    ...
    private DataUsageDetails calculateDataUsage(List<DataPass> dataPasses, final SubscriberInfo info,
                                                BillingCycle billingCycle, LocalDateTime billingStartDate, LocalDateTime billingEndDate) {
        ...
        if (BillingUtils.isUnlimitedUsage(dataPasses)) {
            dataPasses.stream().filter(e -> Arrays.asList("C", "E", "S").contains(e.getInfoType()))
            ...
            // Calculate for Not Unlimited Pass -- Shared Pass       dataUsedUnlimited =
            dataUsedShared = dataPasses.stream().filter(p -> BillingUtils.isSharedPass(info, p))
                    .map(e -> e.getShareDetails().getSharerDataUsage()).flatMap(Collection::stream)
                    .filter(sha -> info.getMsisdn().equals(sha.getMsisdn()))
                    .mapToDouble(SharerDataUsage::getUsedVolume).sum();
        ...
        return DataUsageDetails.builder()
                ...
                .zeroRatedDataUsed(zeroRatedDataUsed != 0D ? zeroRatedDataUsed / 1024 : 0D).build();
    }

    private double getUsageDetails(List<DataPass> dataPasses, Predicate<? super DataPass> predicate) {

        return dataPasses.stream().filter(predicate).mapToDouble(DataPass::getVolume).sum();
    }
